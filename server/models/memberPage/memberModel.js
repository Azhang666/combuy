...
const MemberModel = {
  ...
  pwdChangeAPI: async (uid, data) => {
    ...
    try {
      ...
      if (result1.length == 1) {
        if (
          Math.floor(new Date() - result1[0].change_pwd_time) /
            (1000 * 3600 * 24) <
          14
        ) {
  ...
  editCardAPI: async (uid, data) => {
    const { card_1, card_2, card_3, card_4, expiry_date, security_code, cid } =
      data;
    if (
      card_1.length != 4 ||
      card_2.length != 4 ||
  ...
  collectRender: async (uid, page) => {
    try {
      const sql1 =
        "SELECT COUNT(*) AS total FROM collect WHERE collect.user_id = ?";
      const result1 = await conn.queryAsync(sql1, [uid]);
  ...
  collectProdAPI: async (uid, prod_id, spec_id) => {
    try {
      const sql1 =
        "SELECT count(*) AS count FROM vw_products_info WHERE prod_id = ? AND spec_id =?";
  ...
  commentEditRender: async (uid, order_id, prod_id, spec_id) => {
    ...
    try {
      ...
      for (data of result2) {
        data.comment_time = data.comment_time
  ...
  commentUpdateAPI: async (uid, data) => {
    ...
    console.log("---------", content.substr(0, 249));
    sql1 =
    ...
    try {
      ...
      if (result.length === 0) {
        ...
        for (let data of result) {
          ...
          data.pay_method_nmae =
            data.pay_method === 0 ? "銀行或郵局轉帳" : "信用卡(一次付清)";
  ...
  getOrderDetail: async (uid, order_id) => {
    try {
      const sql =
        'SELECT * FROM vw_order_detail WHERE user_id = ? AND (order_id = ? OR "" = ? ) ';
  ...
  getCommentDetail: async (uid, page) => {
    try {
      const sql =
        'SELECT * FROM vw_comment WHERE user_id = ? AND comment <> "" LIMIT 5 OFFSET ?';
      const result = await conn.queryAsync(sql, [uid, page * 5]);
      for (let data of result) {
        data.order_id = data.order_id.toString().padStart(8, "0");
        data.comment_time = new Date(data.comment_time)
          .toLocaleString()
          .replace(/ /g, "<br/>");
      }
      return result;
