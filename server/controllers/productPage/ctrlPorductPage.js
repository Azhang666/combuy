...
exports.ctrlProdPage = (req, res) => {
  ...
  db.getBrandData([], brandData => {
    //product
    db.getprodItemData(
      [getBrand, getPriceRange.form, getPriceRange.to, offset, productItem, filter],
      productData => {
        ...
        db.getAllProdItemNumData(
          ...
          productTotalData => {
            db.getTagprodItemData(
              [prodSelTag, getPriceRange.form, getPriceRange.to, offset, productItem, filter],
              productTagData => {
                db.getAllTagProdItemNumData(
                  [prodSelTag, getPriceRange.form, getPriceRange.to],
                  async productTagTotalData => {
                    let lastPage = Math.ceil(productTotalData[0].productTotal / productItem)
