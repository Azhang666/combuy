...
exports.getProductsAllData = async (req, res) => {
  ...
  try {
    ...
    if (productResults.length > 0) {
      ...
      const brandResults = await dbQuery(
        "SELECT * FROM brand WHERE brand_id = ?",
        [brandId]
      );
      productResults[0].brandName =
        brandResults.length > 0 ? brandResults[0].brand : null;
    ...
    const imagesResults = await dbQuery(
      "SELECT * FROM product_images_test WHERE prod_id = ?",
      [prodId]
    );
    const sellspecResults = await dbQuery(
      "SELECT * FROM sellspec WHERE prod_id = ?",
      [prodId]
    );
...
exports.partiallyUpdateProduct = (req, res) => {
  ...
  for (let key in sellspecData) {
    if (sellspecMapping[key]) {
      let value =
        key === "price" ? Number(sellspecData[key]) : sellspecData[key];
  ...
  db.getConnection(async (err, connection) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "內部伺服器錯誤", error: err.message });
    }
    try {
      // 使用 promisify 後的查詢函數
