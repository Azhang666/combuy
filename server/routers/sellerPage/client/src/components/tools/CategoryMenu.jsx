...
function CategoryMenu(props) {
    ...
    const fetchMainCategories = async () => {
        try {
            const response = await axios.get('http://localhost:2407/category');
    ...
    const fetchBrands = async () => {
        try {
            const response = await axios.get('http://localhost:2407/brand');
            if (response.data && Array.isArray(response.data)) {
    ...
    const toggleCategories = () => {
        ...
        props.onCategorySelect(null, null);  // 加入這行
    };
